...
test('GIP-003 preserves all task provenance and exposes only residuals', async () => {
  ...
  assert.equal(taskMap.evidence_reconciled_through, 'origin/main@41aeeded8a2aaf51b906d87d953da6acfd765a4b')
  ...
  assert.deepEqual(taskMap.execution_scheduler.ready_task_ids, ['T-054', 'T-056'])
...
test('source reconciliation and execution manifest agree with the governed queue', async () => {
  ...
  assert.equal(ledger.counts.executable_tasks, 14)
  ...
  assert.match(gip003.validation, /ready frontier is exactly T-054 and T-056/)
  assert.match(gip003.validation, /serialized handler order is T-044, T-053, T-059, T-074/)
  assert.doesNotMatch(gip003.validation, /50 implemented|21 executable residuals|53 implemented|18 executable residuals|56 implemented|15 executable residuals/)
  ...
  assert.match(markdownPlan, /Remaining write set:\*\* T-054 and T-056 own `workers\/quests\/src\/page\/scenes\/tools\.ts`/)
