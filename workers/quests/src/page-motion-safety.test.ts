...
import {
  DESKTOP_PAGE_BROWSER_PROOF_PATHS,
  DESKTOP_PAGE_BROWSER_STORY_MATRIX,
  VIEWPORT_PROOF_CAPTURE_STEPS,
  selectDesktopViewportProofCaptureSteps,
...
const BROWSER_STORY_MATRIX = [
  {
    id: 'mission-layout-mobile',
  ...
  {
    id: 'mission-sheet-actions-mobile',
    surface: 'mobile-browser',
    scene: 'mission',
    paths: [
      'sheet-mission-review-gate-mobile.png',
      'sheet-mission-open-proof-mobile.png',
    ],
  },
  {
    id: 'mission-gesture-selection-mobile',
    surface: 'mobile-browser',
    scene: 'mission',
    paths: ['mission-vantyx-selected-mobile.png'],
    requiresGesture: true,
    requiresKeyboard: true,
  },
  {
    id: 'gate-layout-mobile',
    surface: 'mobile-browser',
    scene: 'gate',
    paths: [
      'gate-empty-mobile.png',
      'gate-consequence-mobile.png',
      'gate-iverif-action-request-mobile.png',
      'gate-iverif-queued-proof-mobile.png',
    ],
  },
  {
    id: 'gate-sheet-journeys-mobile',
    surface: 'mobile-browser',
    scene: 'gate',
    paths: [
      'sheet-gate-queued-proof-detail-mobile.png',
      'sheet-gate-confirm-action-request-preflight-mobile.png',
      'sheet-gate-confirm-action-request-request-sent-mobile.png',
      'sheet-gate-approve-preflight-mobile.png',
      'sheet-gate-reroll-preflight-mobile.png',
    ],
  },
  {
    id: 'tools-browser-journeys-mobile',
    surface: 'mobile-browser',
    scene: 'tools',
    paths: [
      'tools-mobile.png',
      'sheet-tools-handoff-action-mobile.png',
    ],
  },
  {
    id: 'story-browser-journeys-mobile',
    surface: 'mobile-browser',
    scene: 'story',
    paths: [
      'story-feed-mobile.png',
      'story-iverif-action-request-mobile.png',
    ],
  },
  {
    id: 'inspect-layout-mobile',
    surface: 'mobile-browser',
    scene: 'inspect',
    paths: [
      'inspect-proof-320-mobile.png',
      'inspect-system-overview-mobile.png',
      'inspect-tapestry-audit-mobile.png',
      'inspect-no-fake-progress-mobile.png',
      'inspect-policy-gap-mobile.png',
      'inspect-live-proof-mobile.png',
      'inspect-gate-priority-mobile.png',
      'inspect-skill-promotion-mobile.png',
      'inspect-iverif-action-request-mobile.png',
      'inspect-companions-mobile.png',
    ],
    requiresKeyboard: true,
  },
  {
    id: 'inspect-sheet-journeys-mobile',
    surface: 'mobile-browser',
    scene: 'inspect',
    paths: [
      'sheet-inspect-skill-promotion-mobile.png',
      'inspect-mira-relationship-mobile.png',
    ],
  },
  {
    id: 'operating-fabric-tabbed-mobile',
    surface: 'mobile-browser',
  ...
  {
    id: 'operating-fabric-sheet-mobile',
    surface: 'mobile-browser',
...
function browserStoryCaptureSteps(row: (typeof BROWSER_STORY_MATRIX)[number]) {
  if (row.surface !== 'mobile-browser') return [];
  return row.paths.map((path) => {
    const step = VIEWPORT_PROOF_CAPTURE_STEPS.find((entry) => entry.path === path);
    assert.ok(step, `${row.id} is missing browser proof step ${path}`);
    assert.equal(step.scene, row.scene, `${row.id} keeps ${path} on the ${row.scene} journey`);
    const selected = selectViewportProofCaptureSteps({ proofPathFilter: path });
    assert.ok(selected.some((entry) => entry.path === path), `${row.id} filter still exposes the exact step for ${path}`);
    return step;
  });
}

function stepJourneyText(step: Record<string, unknown>) {
  return [
    step.expression,
    step.prepareExpression,
    step.waitAfterExpression,
    step.assertExpression,
  ].filter((value): value is string => typeof value === 'string' && value.length > 0).join(' ');
}

function stepHasKeyboardJourney(step: Record<string, unknown>) {
  const text = stepJourneyText(step);
  return /document\.activeElement|aria-selected|tab\.focus\(\)/.test(text);
}

function stepHasDesktopViewport(step: Record<string, unknown>) {
  const viewport = step.viewport as { width?: unknown } | undefined;
  return Number(viewport?.width || 390) >= 1024;
}

function stepHasBrowserAssertion(step: Record<string, unknown>) {
  return typeof step.assertExpression === 'string' && step.assertExpression.length > 0;
}

function stepHasPointerJourney(step: Record<string, unknown>) {
  return typeof step.pointerTargetSelector === 'string' && step.pointerTargetSelector.length > 0;
}
...
test('T-037 · browser story matrix names the shipped mobile browser journeys and keeps desktop out of scope here', () => {
  const ids = BROWSER_STORY_MATRIX.map((row) => row.id);
  assert.equal(ids.length, new Set(ids).size, 'browser story matrix ids stay unique');
  assert.ok(BROWSER_STORY_MATRIX.some((row) => row.surface === 'mobile-browser'), 'matrix keeps mobile browser journeys');
  assert.equal(BROWSER_STORY_MATRIX.every((row) => row.surface === 'mobile-browser'), true, 'owned matrix stays limited to real PAGE mobile browser journeys');
});

test('T-037 · browser story matrix matches the shipped viewport gesture and keyboard stories', () => {
  const captureSteps = selectViewportProofCaptureSteps();
  assert.ok(captureSteps.length > 0, 'viewport proof harness still serves browser stories');
  assert.equal(captureSteps.length, VIEWPORT_PROOF_CAPTURE_STEPS.length - DESKTOP_PAGE_BROWSER_PROOF_PATHS.length, 'default selection keeps the canonical browser proof set and excludes diagnostic desktop captures');
  assert.equal(captureSteps.some((step) => DESKTOP_PAGE_BROWSER_PROOF_PATHS.includes(step.path)), false, 'default selection cannot rewrite diagnostic desktop captures into canonical evidence');
  for (const row of BROWSER_STORY_MATRIX.filter((entry) => entry.surface === 'mobile-browser')) {
    const steps = browserStoryCaptureSteps(row);
    assert.ok(steps.length > 0, `${row.id} resolves to at least one browser proof step`);
    if (row.requiresGesture) assert.ok(steps.some((step) => step.touchDragTargetSelector), `${row.id} keeps a touch-drag gesture path`);
    if (row.requiresKeyboard) assert.ok(steps.some(stepHasKeyboardJourney), `${row.id} keeps a keyboard/focus journey assertion`);
  }
});

test('T-037 · desktop browser journey matrix names the shipped PAGE desktop journeys', () => {
  const ids = DESKTOP_PAGE_BROWSER_STORY_MATRIX.map((row) => row.id);
  const journeys = DESKTOP_PAGE_BROWSER_STORY_MATRIX.map((row) => row.journey);
  assert.equal(ids.length, new Set(ids).size, 'desktop browser matrix ids stay unique');
  assert.equal(DESKTOP_PAGE_BROWSER_STORY_MATRIX.every((row) => row.surface === 'desktop-browser'), true, 'desktop matrix stays limited to desktop browser journeys');
  assert.deepEqual(journeys, ['mission', 'gate', 'tools', 'story', 'inspect', 'portfolio'], 'desktop browser matrix names the six PAGE desktop journeys');
});

test('T-037 · desktop browser selector resolves only real desktop proof steps with assertions and interaction coverage', () => {
  const captureSteps = selectDesktopViewportProofCaptureSteps();
  assert.deepEqual(
    captureSteps.map((step) => step.path),
    DESKTOP_PAGE_BROWSER_PROOF_PATHS,
    'desktop selector exposes the declared desktop proof paths in order',
  );
  for (const row of DESKTOP_PAGE_BROWSER_STORY_MATRIX) {
    const steps = row.paths.map((path) => {
      const step = VIEWPORT_PROOF_CAPTURE_STEPS.find((entry) => entry.path === path);
      assert.ok(step, `${row.id} is missing browser proof step ${path}`);
      const selected = selectViewportProofCaptureSteps({ proofPathFilter: path });
      assert.ok(selected.some((entry) => entry.path === path), `${row.id} filter still exposes the exact step for ${path}`);
    ...
    assert.ok(steps.every(stepHasDesktopViewport), `${row.id} keeps every step on a desktop viewport`);
    assert.ok(steps.every(stepHasBrowserAssertion), `${row.id} keeps browser assertions on every desktop step`);
    if (row.requiresKeyboard) assert.ok(steps.some(stepHasKeyboardJourney), `${row.id} keeps a keyboard/focus desktop journey assertion`);
    if (row.requiresPointer) assert.ok(steps.some(stepHasPointerJourney), `${row.id} keeps a pointer desktop journey assertion`);
  }
});

test('T-037 · browser story matrix keeps the real PAGE mobile browser proof entrypoint only', () => {
  const mobileLikeSteps = VIEWPORT_PROOF_CAPTURE_STEPS.filter((step) => {
    const width = Number((step.viewport && step.viewport.width) || 390);
    return width <= 430 && /mobile/i.test(String(step.path || ''));
  });
  assert.ok(mobileLikeSteps.length > 0, 'real PAGE browser harness keeps mobile proof steps');
});
