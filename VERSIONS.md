...
ruleset and reconciles portfolio-origin issue #290. The dated audit resolves the
`reddit-cli` alias to `Sheshiyer/reddit-flux`, assigns Brandmint Showcase to the
Meristem program from registry and architecture evidence, makes the Symphonics
repository-only role exact, and preserves seven unmatched rows as explicit
holds without catalog or filesystem mutation.
