...
> **On the parallel Arcana line** (2026-08-26) — the sister project **Temperance Engine** at `~/.temperance_engine/` (the execution runtime that consumes Cambium's admitted assignments) uses a different release-naming convention: the **22 Major Arcana** for majors + **56 Minor Arcana** for minors (suit = concern — Wands architecture · Cups memory · Swords routing · Pentacles evidence). Per its [`ARCANA-NOMENCLATURE.md`](../../../.temperance_engine/ARCANA-NOMENCLATURE.md), the mapping isn't decorative: **v14 = XIV Temperance because the engine IS Temperance** — the whole runtime IS the tempering discipline. Not a collision with the Muses — two release lines, two conventions, one for each project's edge. Cambium releases honour memory (Mnemosyne's daughters, art of memory — fitting an operator whose whole edge is *remembering*); Temperance Engine releases honour proportion (Arcana, right proportion under load — fitting an operator whose whole edge is *keeping combos honest*). Cross-references land in each project's CHANGELOG when a shipped item on one side unblocks or requires work on the other. As of 2026-08-26, Temperance Engine has shipped v4 · XIV Temperance (Plugin Contract v1), v4.1 · Ace of Wands (45 cluster orchestrator agents sourced from `/Volumes/madara/2026/Projects/thoughtseed/skill-clusters/`), v4.2 · Two of Wands (dispatch advisory + alchemy map v3), v4.3 · Three of Wands (SPRD-03 combo-diversity fix), v5.0 · XVII The Star (canonical banner foundation), v5.1 · Ace of Swords (7 remaining surface adapters + 6 `/te` slash commands + Adaptive Island state machine), v5.2 · Four of Swords (dual-surface stabilization — Cambium's `apps/portfolio-cartographer` + Speculum browser UI stay as opt-in surfaces alongside the new terminal default).
...
The same planning wave routes five active non-template branch packets into
their exact owning repositories through bounded open issues. Cambium retains
cross-portfolio sequencing; Fitcheck, Vantyx, IVerif, DLOCK, and Snow Gloves
OS now own their local residual implementation and proof queues.
