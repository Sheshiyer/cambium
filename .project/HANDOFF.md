...
- T-07 invariant: `changedPathsAndKinds()` includes `git ls-files --others --exclude-standard` (untracked, non-gitignored files). Machine-local files with `/Volumes/` paths now gitignored: `.superset/`, `.temperance/manifest.json`, `.temperance/goal.json`.
