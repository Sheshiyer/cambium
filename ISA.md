...
- 2026-08-16 18:31: T-059 moves Story event projection to the public Worker boundary. RED proved that durable receipt, founder-decision, and completed-transition facts were absent from canonical beats while the browser could add a duplicate legacy ActionRequest row. GREEN now emits only exact receipt-backed facts, marks the stream authoritative, and rejects ambiguous WorkObject joins, missing receipts, malformed event times, and secret-like public text. Focused handler-to-renderer tests pass 2/2 and the full handler suite passes 383/383; broader candidate verification remains pending.
- 2026-08-16 18:39: T-059 candidate verification passes reconciliation 2/2, core 1761/1761, validation, rendered-doc parity, diff checks, and the canonical browser matrix with 27 layout plus 20 clickability proofs. The first release attempt reached R3F and stopped only because ignored `node_modules` was absent in the fresh worktree; after restoring lockfile-pinned dependencies, the exact committed-candidate release gate passed retention 177/177, readiness 38/38, the 15-capture mobile contract, R3F 99/99 plus production build, and desktop packaging 5/5.
