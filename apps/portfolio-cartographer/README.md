...
A founder-only hosted admin workbench for mapping two shallow portfolio roots,
reconciling repository identity, origin, WorkObject grammar, and planning
authority, then saving bounded intake actions through the Cambium Worker.

The artifact contains the verified 72-object Thoughtseed catalog snapshot, a privacy-safe
44-reference repository-evidence snapshot, and the five canonical organ
workflows. It also contains a proposal-only root map for 47 Thoughtseed folders,
30 active Tryambakam · Noesis Projects, separate archive/worktree infrastructure,
and the R2-synced Thoughtseed vault copy. Seven smart views reveal ongoing, paused,
white-labelable, review-needed, unplanned, and historical work. Browser storage
is a recovery draft, never the durable authority.
...
Use the portfolio selector to open Tryambakam · Noesis. Its unit is `Project`,
not `Client Branch`; every card is intake-only until exact GitHub identity and
repository-owned planning are ingested.
...
3. only after durable evidence exists, the Worker creates a bounded
   `pending-governed-intake` trigger in the action queue;
