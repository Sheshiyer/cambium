...
test('Fitcheck opens one authority-honest golden-path Operate view', async () => {
  const source = await readFile(new URL('./App.tsx', import.meta.url), 'utf8')
  const styles = await readFile(new URL('./index.css', import.meta.url), 'utf8')
  ...
  assert.match(source, /function FitcheckOperate/)
  assert.match(source, /data-fitcheck-authority="packet-plan"/)
  assert.match(source, /packet plan · not D1 tasks/)
  assert.match(source, /Mapped and planned are evidenced/)
  ...
  assert.match(source, /id === FITCHECK_GOLDEN_PATH\.identity\.workId \? 'operate' : 'intake'/)
  assert.match(source, /initial\.focusedId === FITCHECK_GOLDEN_PATH\.identity\.workId \? 'operate' : 'intake'/)
