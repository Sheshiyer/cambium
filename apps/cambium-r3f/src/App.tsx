...
import { defaultScreenId, productScreenOrder, screenOrder } from './scene/route-registry';
...
export function App() {
  ...
  const scene = useMemo(() => buildCambiumScene(activeScreenId, cameraMode, devEnabled), [activeScreenId, cameraMode]);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const currentIndex = visibleScreenOrder.indexOf(activeScreenId);
      if (event.key === 'ArrowRight') {
        setScreen(visibleScreenOrder[(currentIndex + 1) % visibleScreenOrder.length]);
      }
      if (event.key === 'ArrowLeft') {
        setScreen(visibleScreenOrder[(currentIndex - 1 + visibleScreenOrder.length) % visibleScreenOrder.length]);
      }
