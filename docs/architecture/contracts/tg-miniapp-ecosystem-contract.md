...
Gate renders the public `thoughtseed.action-request-list-item.v1` projection
without fixture-only enrichment. A Telegram-originated row may display:

- `id`, `branchId`, and `questId`: durable request and branch context.
- `topic.topicKey`, `topic.threadId`, and optional `topic.sourceMessageId`:
  source provenance only. `sourceMessageId` does not imply that the Mini App is
  inside the source message or that a receipt will edit it.
...
- `receipts.latest`: the latest actual redacted receipt, never a predicted
  `receiptExpectation` authored only by a fixture.
- `evidence`, `consequence`, `reversibility`, and `idempotencyHint`.

`needs_signed_confirmation` renders one Mini App signed-confirm control.
`queued` renders the selected option, latest receipt, and operator-waiting state
without another confirm control. `consumed`, `completed`, and `superseded` leave
the active Gate list.

The authenticated operator queue lists both generic gate actions and queued
ActionRequests. An ActionRequest consume is keyed by durable request `id`, adds
one redacted consume receipt, is idempotent on replay, and performs no external
send, spend, deployment, or client mutation.

The Mini App may show topic/thread/message provenance, but it must not show raw
founder ids, raw `initData`, bearer tokens, private chat ids, or queued secrets.
...
- Viewport diagnostics: temporal CI/local artifacts; never canonical proof
- Live readiness manifest: temporal CI/local artifact generated by `npm run proof:tg-live-readiness`
- Worker list proof: `.artifacts/tg-miniapp-live-proof/worker-network-probe.json`
- Device proof target: `.artifacts/tg-miniapp-live-proof/telegram-webview.json`
- Signed smoke target: `.artifacts/tg-miniapp-live-proof/signed-action-smoke.json`
