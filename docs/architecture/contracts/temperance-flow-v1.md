...
2. `gsd_state` — the live transition and canonical command in
   `.planning/STATE.md`.
3. `active_plan` — the first plan anchors one dependency-ready remaining-phase
   unit. Every incomplete plan is also source-digested into that unit. Internal
   `<task>` blocks are plan instructions, not independently dispatched units.
...
- `ready`: exactly one remaining-phase unit and exactly one canonical GSD phase
  command, with no blocked reasons. The unit binds every incomplete plan that
  the phase command can execute.
...
adapter supplies a `verified`, `fresh` result bound to the exact remaining-phase unit, command,
