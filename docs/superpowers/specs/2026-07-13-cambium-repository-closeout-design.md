...
2. Review and merge the only unique semantic working-tree change as one focused two-file pull request.
...
- `git diff --name-only origin/main...HEAD` returns exactly those two paths;
...
The commit, pull-request title, and pull-request body must not contain GitHub issue-closing keywords. The pull request links no issue because the verified open-issue snapshot is empty.

## Merge gate

For each approved pull request:

1. Refresh the GitHub `main` SHA and update the branch if its base is stale.
2. Confirm the diff matches that pull request's declared file boundary.
