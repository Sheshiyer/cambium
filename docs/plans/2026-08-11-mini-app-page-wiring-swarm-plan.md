...
- **Landed T-060/T-061 boundary:** exact replay collapses by stable event identity plus canonical event fingerprint; conflicting reuse of one event identity fails closed. Every canonical timeline segment and Story card exposes exact event time, WorkObject identity and kind, source, receipt, and event kind when supplied; legacy served rows label themselves as legacy events without inventing a kind or provenance.
