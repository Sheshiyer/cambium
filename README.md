...
- **[INFINITE-GAME.md](./INFINITE-GAME.md)** + **[`docs/infinite-game-architecture.html`](./docs/infinite-game-architecture.html)** · **[`docs/infinite-game-flowchart.html`](./docs/infinite-game-flowchart.html)** — the **infinite-game operator**: the organs are *finite-game players* (run → converge → dormant ✅) nested inside a *never-terminating operator* whose physics is **viability + allostasis + co-evolution** (Banach demoted to the finite runs); it optimizes the **vision→mission→goal cascade** so revenue/growth fall out downstream (solvency = a bound), via the **micro/meso/macro wake loop** + the **mid-brain → noesis** bypass.
- **[ONBOARDING-OCTALYSIS.md](./ONBOARDING-OCTALYSIS.md)** + **[`docs/onboarding-octalysis.html`](./docs/onboarding-octalysis.html)** — the **first session**: the 20-interaction onboarding tutorial mapped to the **Octalysis** 8 drives (White-Hat-dominant; black hat only when grounded; **mid-brain → noesis**).
- **[`bin/operator/`](./bin/operator/)** — the **operator** (thin · TypeScript · zero-dep, Node-native `.ts`): the event-sourced **wake loop** + the micro/meso/macro router + the 2 NPC stubs, reusing the cortex / gate / why-handler seams. Try it: `npm run operator demo` · `npm test`.
- **[`docs/organs.html`](./docs/organs.html)** — **per-organ structure**: each organ (genesis · hands · taste · will · cortex) rendered as the *same* self-similar Φ with its real internals (waves, the 40 clusters, the 7-agent C-suite, the NIM) — plus a **dedicated full page per organ** in [`docs/organs/`](./docs/organs/) (genesis · hands · taste · will · cortex). These pages are **generated from source** by `npm run render-docs` — the component lists + counts are read *live* from the organ repos, so they can't drift; `npm run render-docs:check` is the drift guard (homeostasis applied to the docs themselves). The detail behind the tapestry diagram.
