...
- **If required fields are missing**: the contract fails closed at `genesis`; downstream stages must refuse
  to infer the missing brand core and fall back to reroll / repair instead of improvising.
...
- **If required fields are missing**: `build` cannot ship page sections or campaigns as canonical output;
  the stage must mark the seam incomplete rather than synthesizing new messaging from scratch.
...
- **If required fields are missing**: `taste` cannot score on-brandness reliably and `build` must not invent
  the aesthetic system; the hand-off is treated as contract drift.
...
- **If required fields are missing**: `build` may not assume which assets exist or should be generated; it
  must stop the asset branch and surface the missing plan.
...
- **If required fields are missing**: `build` must not hallucinate IA or screen flow; it should fail the
  section contract and request a seeded plan.
...
- **If required fields are missing**: downstream stages must default to safe static behavior or halt the
  interaction-specific branch; they must not invent motion logic that changes product meaning.
...
- **If required fields are missing**: the system cannot distinguish deliberate change from drift, so the
  seam fails closed and routes to verification / reroll instead of release.
...
today; the full chain runs once the gated stages (genesis, taste) are approved. The invariant now covers
both stage order **and** the variable contract vocabulary above: required seeded variables must either be
present and passed forward, or the seam fails closed.
