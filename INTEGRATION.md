...
| **I2a** | genesis **as a service** | `idea` → `brand-dna` on demand | 🟡 **wired (gated)** — `adapters.json` + `compose run`; spawn `--approve`-gated, and the response must carry the variable contracts (`brand_system`, `copy_slots`, `visual_system`) rather than prose-only output | brandmint ← conductor |
...
| **I2c** | taste **as a service** | `brand-dna` → `taste-brief` + on-brand verdict | 🟡 **wired (gated)** — `adapters.json` + `compose run`; spawn `--approve`-gated, and emit variable contracts for downstream build / ops, not just an English brief | skill-clusters taste-resolve ← conductor |
...
its stage contract **and passing the canonical variable contracts forward as structured data**. **I3**
unifies the memory the paid tier is built on. **I4** then decides error-vs-intent against those same
variables. **B1–B3** are the engine lessons from the first real brand run
