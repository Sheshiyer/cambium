...
| **I2a** | genesis **as a service** | `idea` → `brand-dna` on demand | 🟡 **wired (gated)** — `adapters.json` + `compose run`; spawn is `--approve`-gated, and the wiring should evolve to carry the documented variable contracts (`brand_system`, `copy_slots`, `visual_system`) rather than prose-only output | brandmint ← conductor |
