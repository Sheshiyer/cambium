...
| **I3** | unify the NIM cortex | one aesthetic memory across all organs | 🟡 **Worker ingestion contract implemented** — `POST /v1/context/cortex-ingest` validates, chunks, embeds, and idempotently upserts bounded metadata into `CAMBIUM_CORTEX`; activation still requires the dedicated Worker secret, a reviewed production deployment, and the Hermes caller. `bin/lib/cortex.mjs` remains the provider-neutral local interface. | Cambium Worker + Hermes caller |
