...
**Status:** repository-first v4, portfolio-ingestion headers, and hosted founder action receipts are implemented; the Cambium repository packet is reviewed-held, and production promotion proceeds only through issue #293's exact Worker Version and rollback gate.
...
3. Unplanned cards replace horizon shortcuts with one `Review repository & map` action (accessible name `Inspect & reconcile`); Plan controls unlock only after repository, origin, authority, review, and canonical-type agreement gates pass.
...
7. Focused/domain/generator/action/route tests, same-origin hosted-action audit, bundle/embed parity, responsive local browser proof, full release verification, independent review, and ReReadCheck pass.
...
**Portfolio-ingestion checkpoint (2026-08-07):** `docs/project-management/portfolio-roots.v1.json` freezes 47 Thoughtseed folder proposals and 30 Tryambakam · Noesis Projects under the shallow `<projects-root>/<portfolio>/<repository>` grammar. The destination roots now carry reversible `PORTFOLIO.md` and `portfolio-map.v1.json` headers only; no repository moved or nested, and the R2-synced `thoughtseed-labs` vault remains infrastructure rather than a WorkObject folder. Root-map digest: `588f136a14cac55dbba30b11394288943c56bfebba2b700b4c2d25590747c52b`. GitHub tracking: issue #295 and Project #14.

**Hosted admin-action checkpoint (2026-08-07):** the offline export/import header is retired. The founder-only Workbench posts one closed-schema action to `/v1/admin/portfolio/actions`; Cloudflare Access/Plexus or Telegram signed-founder authentication is mandatory. The Worker records immutable R2 evidence before a bounded pending-intake trigger, returns idempotent receipts, and never writes the Goal Graph. Thoughtseed actions queue repository reconciliation; Tryambakam actions queue Project repository ingestion. Design and execution: `docs/plans/2026-08-07-portfolio-hosted-admin-actions-design.md` and `docs/plans/2026-08-07-portfolio-hosted-admin-actions-implementation.md`. Tracking: issue #296 in Project #14. Packet review #292 is cleared; production remains gated by #293.

**Thoughtseed governed-project-birth refinement (2026-08-07):** the founder retired Tryambakam · Noesis from the active Workbench while preserving its reviewed root evidence on disk. The active UI now targets Thoughtseed only. One typed birth contract accepts local-founder, agent, RBAC, dgchat, or system intent; explicit local-founder commands may execute after local validation, while every non-founder source remains Founder-Gate-pending until the Worker resolves an authoritative Thoughtseed Gate record that binds the exact intent digest. The standalone CLI is local-founder-only unless a trusted host injects that resolver. A trusted local executor pins the reviewed root/catalog digests, derives the shallow `thoughtseed/<repository>` destination, initializes the Git/project/workflow packet, and writes project-local `pending-cambium-ingestion` plus index-proposal receipts. It does not create a GitHub remote, mutate the canonical portfolio registry, write the Vault/R2 copy, touch the Goal Graph, or create project folders during production promotion. Contract: `docs/project-management/thoughtseed-project-birth.v1.json`. Design and execution: `docs/plans/2026-08-07-thoughtseed-governed-project-birth-design.md` and `docs/plans/2026-08-07-thoughtseed-governed-project-birth-implementation.md`. Packet review #292 is cleared; production remains gated by #293.

**Thoughtseed project-closeout refinement (2026-08-08):** the Workbench now has a terminal `Completed / Closed` status for work that no longer needs active tracking. Closeout is receipt-backed, not a cosmetic signal: the founder must prepare a final handoff summary, closeout receipt JSON, agent-aware active/finished memory projection, active-to-finished index delta, R2 vault archive manifest, and downstream-flow stop/transfer confirmation before the item leaves active views. The hosted action `close-work-object` writes immutable R2 evidence before queueing the `project-closeout` flow; the trusted local executor appends `.project/HANDOFF.md` and writes `.project/project-closeout-receipt.v1.json`, `.project/agent-memory-projection.v1.json`, and `.project/finished-index-delta.v1.json` without moving folders, mutating the Vault/R2 copy, closing GitHub issues, touching the Goal Graph, or deploying production. Contract: `docs/project-management/thoughtseed-project-closeout.v1.json`. Design and execution: `docs/plans/2026-08-08-thoughtseed-project-closeout-workflow-design.md` and `docs/plans/2026-08-08-thoughtseed-project-closeout-workflow-implementation.md`. Production remains gated by #293.
