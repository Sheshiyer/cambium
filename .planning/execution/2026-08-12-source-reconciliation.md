...
| #285 | continuity: preserve and close the unfinished brand/CF/Hermes board | residual | missingAuthority: Board closure is not present in merged issue-closure commits and remains as historical continuity context in planning documents. |
| #287 | relocation: prepare Cambium Git-graph move and portfolio reconciliation | blocked | missingAuthority: Relocation execution requires explicit runbook approval and environment authority approvals; none are present in current merge set for this residual lane. |
| #290 | portfolio: audit repository origin and classification mismatches | residual | missingAuthority: Mismatch audit exists as issue context in continuity planning but no merged remediation task set has been executed in origin/main. |
